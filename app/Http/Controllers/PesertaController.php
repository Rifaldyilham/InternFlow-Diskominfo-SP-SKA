...
class PesertaController extends Controller
{
    public function store(StorePesertaRequest $request)
    {
        ...
        // Handle file uploads
        if ($request->hasFile('surat_penempatan')) {
            $path = $request->file('surat_penempatan')->store('peserta/surat_penempatan', 'public');
            $data['surat_penempatan_path'] = $path;
        }

        if ($request->hasFile('cv')) {
            $path = $request->file('cv')->store('peserta/cv', 'public');
            $data['cv_path'] = $path;
        }
